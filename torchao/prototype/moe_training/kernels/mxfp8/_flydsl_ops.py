...
"""Buffer-descriptor and vector ops that exist in flydsl 0.2.4 AND 0.3.0.

WHY THIS FILE EXISTS. The FlyDSL MXFP8 cast kernels in this package were written
against flydsl 0.2.4 and call ``flydsl.expr.buffer_ops`` and
``flydsl.expr.vector``. **0.3.0 deleted both modules.** Because
``kernels/mxfp8/__init__`` imports ``quant``, which imports ``flydsl_utils``,
that made the whole package unimportable on 0.3.0 -- ``import torchao...`` failed
before any kernel was even selected, including for callers that never touch a
FlyDSL kernel.

The fix is not a shim. ``rocdl.MakeBufferRsrcOp``, ``RawPtrBufferLoadOp`` and
``RawPtrBufferStoreOp``, and the ``vector`` MLIR dialect, are all present in BOTH
versions; 0.2.4's ``buffer_ops``/``vector`` wrappers were convenience layers over
exactly those. This is that layer, re-implemented against the ops directly and
kept to what these kernels actually use. One implementation, no version branch:
0.2.4 keeps working and 0.3.0 stops needing deleted code.

What is deliberately NOT here, because no caller needs it: ``mask=``,
``cache_modifier``, ``stride``, ``base_byte_offset``, ``offset_is_bytes``, the
``from_addr`` descriptor constructor, and the memref-derived size fallback.
Adding one back means porting its logic from 0.2.4, not guessing.

THE BOUNDS ARE LOAD-BEARING, not an optimisation. ``num_records`` is what makes
an out-of-range read return 0 rather than fault, and these kernels rely on it:
a tail block's read can run past the plane, where 0 as an e8m0 is 2**-127 and
underflows exactly as the masking intends.
"""
...
def create_buffer_resource(memref_val, max_size: bool = True, *,
    ...
    """
    base_ptr = _fly.extract_aligned_pointer_as_index(ir.Type.parse("!llvm.ptr"),
                                                     _unwrap(memref_val))
    ...
    return rocdl.MakeBufferRsrcOp(
        ir.Type.parse("!llvm.ptr<8>"), base_ptr, _const_i16(0),
        num_records, _const_i32(_CDNA_BUFFER_FLAGS),
    ).result


def buffer_load(rsrc, offset, vec_width: int = 4, dtype=None,
                soffset_bytes: Optional[Union[int, ir.Value]] = None,
                is_scalar: bool = False, cache_modifier: int = 0):
    ...
    if is_scalar:
        ...
        rsrc_v4 = llvm.bitcast(ir.VectorType.get([4], T.i32()),
                               llvm.ptrtoint(ir.IntegerType.get_signless(128),
                                             _unwrap(rsrc)))
    ...
    soffset = (_const_i32(0) if soffset_bytes is None
               else _const_i32(soffset_bytes) if isinstance(soffset_bytes, int)
               else _as_i32(soffset_bytes))
    return rocdl.RawPtrBufferLoadOp(result_type, _unwrap(rsrc), offset, soffset,
                                    _const_i32(cache_modifier)).result


def buffer_store(data, rsrc, offset, cache_modifier: int = 0):
    """Store `data` to `rsrc` at an ELEMENT offset (scaled to bytes here).

    `cache_modifier` is the ROCDL op's `aux` operand -- the same meaning and the
    same position 0.2.4's `buffer_ops.buffer_store` gave it. These kernels use it
    to mark streaming stores, so it is a real argument here rather than the
    hardcoded 0 the GEMM-side copy of this module can afford.
    """
    data = _unwrap(data)
    offset = _const_i32(offset) if isinstance(offset, int) else _as_i32(offset)
    ...
    rocdl.RawPtrBufferStoreOp(data, _unwrap(rsrc), offset, _const_i32(0),
                              _const_i32(cache_modifier))


def vec_broadcast(result_type, source):
    """`vector.broadcast`, replacing 0.3.0's deleted `flydsl.expr.vector`.

    Same reason as `vec_extract` below: 0.3.0 removed `flydsl.expr.vector`
    wholesale, but the MLIR dialect it wrapped is present in both versions, so
    one implementation over `_mlir.dialects.vector` serves 0.2.4 and 0.3.0
    without a version branch. See the module header.
    """
    return _vector.BroadcastOp(result_type, _arith_ext.unwrap(source)).result


def vec_from_elements(result_type, elements):
    """`vector.from_elements`, same rationale as `vec_broadcast`."""
    return _vector.FromElementsOp(
        result_type, [_arith_ext.unwrap(e) for e in elements]).result


def vec_bitcast(result_type, source):
    """`vector.bitcast`, same rationale as `vec_broadcast`."""
    return _vector.BitCastOp(result_type, _arith_ext.unwrap(source)).result
...
def vec_extract(vector, static_position=None, dynamic_position=None):
    ...
    dynamic_position = [_arith_ext.unwrap(i, index=True) for i in (dynamic_position or [])]
    ...
    return _vector.ExtractOp(_arith_ext.unwrap(vector),
                             static_position=static_position,
                             dynamic_position=dynamic_position).result
