...
def get_rocm_mxfp8_grouped_mm_config(
    K: int,
    N: int,
) -> dict[str, int]:
    if K % 256 == 0 and K >= 2560:
        return {"BLOCK_M": 256, "BLOCK_N": 256, "BLOCK_K": 128, "GROUP_M": 2}
    return {"BLOCK_M": 128, "BLOCK_N": 128, "BLOCK_K": 128, "GROUP_M": 8}


def get_rocm_mxfp8_wgrad_config(
    M: int,
    K: int,
    N: int,
) -> dict[str, int]:
    if K % 256 == 0 and N % 128 == 0:
        return {"BLOCK_N": 128, "BLOCK_K": 256, "BLOCK_M": 128, "PERSISTENT": False}
    return {"BLOCK_N": 128, "BLOCK_K": 128, "BLOCK_M": 128, "PERSISTENT": False}


def get_rocm_mxfp8_mm_config(
    K: int,
    N: int,
) -> dict[str, int]:
    if N <= 4096:
        return {"BLOCK_M": 128, "BLOCK_N": 256, "BLOCK_K": 128}
    return {"BLOCK_M": 256, "BLOCK_N": 256, "BLOCK_K": 128}
...
if _rocm_mxfp8_available:
    ...
    def _mxfp8_grouped_mm_kernel(
        ...
        for k_outer in range(0, tl.cdiv(K, BLOCK_K)):
            ...
            # data masked to 0.0, the tail contribution to dot_scaled is 0x1=0.
            a_scale = tl.load(
    ...
    @triton.autotune(configs=_GROUPED_MM_AUTOTUNE_CFGS, key=["M", "N", "K", "E"])
    @triton.jit
    def _mxfp8_grouped_mm_kernel_autotune(
        A_ptr, A_stride_m, A_stride_k,
        B_ptr, B_stride_e, B_stride_n, B_stride_k,
        A_scales_ptr, A_scales_stride_m, A_scales_stride_kb,
        B_scales_ptr, B_scales_stride_e, B_scales_stride_n, B_scales_stride_kb,
    ...
    def triton_mxfp8_grouped_mm_autotune(
        ...
        _mxfp8_grouped_mm_kernel_autotune[grid](
    ...
    def triton_mxfp8_grouped_mm(
        ...
        grid = (
            triton.cdiv(grid_m_bound, BLOCK_M) * triton.cdiv(N, BLOCK_N),
            E,
        )
    ...
    @triton.jit
    def _mxfp8_wgrad_persistent_kernel(
        GO_ptr, GO_stride_n, GO_stride_m,
        GO_scales_ptr, GO_scales_stride_n, GO_scales_stride_mb,
        IA_ptr, IA_stride_k, IA_stride_m,
        IA_scales_ptr, IA_scales_stride_k, IA_scales_stride_mb,
        ...
        num_tiles_k = tl.cdiv(K, BLOCK_K)
        tiles_per_expert = num_tiles_n * num_tiles_k
        num_pid_tiles = tl.num_programs(axis=0)
        ...
        for tile_idx in range(pid_tile, tiles_per_expert, num_pid_tiles):
            pid_n = tile_idx // num_tiles_k
            pid_k = tile_idx % num_tiles_k
    ...
    def _mxfp8_wgrad_split_kernel(
        ...
        pid_g = pid_gs // SPLIT_M
        pid_split = pid_gs % SPLIT_M
    ...
    def _mxfp8_wgrad_reduce_kernel(
        ...
        for split_idx in range(0, SPLIT_M):
            acc += tl.load(
                P_ptr
                + pid_g * P_stride_e
                + split_idx * P_stride_s
                + n_offs[:, None] * P_stride_n
                + k_offs[None, :] * P_stride_k,
    ...
    def triton_mxfp8_wgrad(
        ...
        if PERSISTENT:
            ...
            tiles_per_expert = num_tiles_n * num_tiles_k
            num_sms = torch.cuda.get_device_properties(go_t.device).multi_processor_count
            persistent_ctas = CTAS_PER_EXPERT
            if persistent_ctas <= 0:
                persistent_ctas = max(1, min(tiles_per_expert, num_sms // max(E, 1)))
            else:
                persistent_ctas = max(1, min(tiles_per_expert, persistent_ctas))
            persistent_grid = (persistent_ctas, E)
            _mxfp8_wgrad_persistent_kernel[persistent_grid](
        ...
        partials = torch.empty((E, SPLIT_M, N, K), dtype=torch.float32, device=go_t.device)
        split_grid = (
            triton.cdiv(N, BLOCK_N),
            triton.cdiv(K, BLOCK_K),
            E * SPLIT_M,
        ...
        _mxfp8_wgrad_split_kernel[split_grid](
            ...
            ia_scale.view(torch.uint8),
            ia_scale.stride(0), ia_scale.stride(1),
            partials, partials.stride(0), partials.stride(1), partials.stride(2), partials.stride(3),
            group_end_offsets,
            M, N, K,
            BLOCK_N=BLOCK_N, BLOCK_K=BLOCK_K, BLOCK_M=BLOCK_M,
            SPLIT_M=SPLIT_M,
    ...
    def _mxfp8_mm_kernel(
        ...
        for k_outer in range(0, tl.cdiv(K, BLOCK_K)):
            ...
            # tail contribution to dot_scaled is 0x1=0 regardless.
            a_scale = tl.load(
                A_scales_ptr + m_offs[:, None] * A_scales_stride_m + kb_offs[None, :] * A_scales_stride_kb,
                mask=m_mask[:, None] & kb_mask[None, :], other=127,
            )
